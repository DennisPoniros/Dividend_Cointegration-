...
% Public Version - Conceptual Framework
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\title{%
    ...
    Combined Absolute and Relative Momentum with Yield-Based Position Sizing
}
...
\begin{abstract}
This paper presents a systematic equity trading strategy that combines dual momentum principles with a dividend-quality stock universe and novel yield-based position sizing. The strategy exploits the well-documented momentum anomaly by applying both absolute momentum (time-series) and relative momentum (cross-sectional) filters to a carefully constructed universe of large-capitalization, dividend-paying stocks. We formalize the alpha generation mechanism through a signal-to-noise quality filter and introduce inverse yield-weighted position sizing, where stocks with lower dividend yields receive larger allocations. This weighting scheme leverages the insight that lower yields indicate recent price appreciation, serving as an additional momentum confirmation signal. The strategy employs periodic rebalancing designed to capture intermediate-term momentum while managing transaction costs. We provide theoretical justification grounded in behavioral finance and market microstructure literature, demonstrating how our approach addresses known weaknesses in traditional momentum strategies through universe construction, signal quality filtering, and dividend-informed position sizing.
\end{abstract}
...
This paper addresses these challenges through four methodological innovations:
...
\begin{enumerate}
    ...
    \item \textbf{Inverse yield-weighted sizing}: Allocating larger positions to stocks with lower dividend yields, which serves as momentum confirmation since lower yields indicate recent price appreciation
\end{enumerate}

\subsection{Contribution}

Our contribution is fourfold. First, we formalize the dual momentum approach within a rigorous mathematical framework, providing explicit decision rules that can be implemented systematically. Second, we demonstrate how universe construction---specifically, focusing on dividend-paying stocks---can improve momentum strategy characteristics by reducing volatility and improving signal quality. Third, we introduce a signal-to-noise filter based on the momentum-to-volatility ratio that systematically eliminates false signals. Fourth, we develop inverse yield-weighted position sizing that uses dividend yield as a momentum confirmation signal, amplifying exposure to stocks exhibiting the strongest price appreciation.

\subsection{Paper Organization}

The remainder of this paper is organized as follows. Section \ref{sec:literature} reviews the relevant academic literature. Section \ref{sec:universe} details our universe construction methodology. Section \ref{sec:methodology} presents the conceptual algorithm specification. Section \ref{sec:alpha} analyzes the sources of alpha generation. Section \ref{sec:theory} provides theoretical justification. Section \ref{sec:risk} discusses risk considerations. Section \ref{sec:conclusion} concludes.
...
The momentum effect was first systematically documented by \citet{jegadeesh1993returns}, who showed that buying past winners and selling past losers generates significant abnormal returns over intermediate horizons. Subsequent research has confirmed the effect's robustness:

\begin{itemize}
    \item \textbf{International evidence}: \citet{rouwenhorst1998international} documented momentum in multiple European markets
...
\begin{itemize}
    ...
    \item The dividend aristocrats (companies with long histories of consecutive dividend increases) outperform on a risk-adjusted basis \citep{siegel2005future}
\end{itemize}
...
\midrule
Minimum Market Capitalization & Large-cap stability and liquidity \\
Minimum Dividend History & Demonstrated commitment to shareholders \\
Minimum Average Daily Volume & Execution liquidity \\
\bottomrule
\end{tabular}
\end{table}

The specific thresholds for each criterion are calibrated to balance universe breadth with quality requirements.
...
Applying these criteria yields a diversified universe spanning all major sectors, with meaningful representation in Financials, Industrials, Consumer Staples, Healthcare, and Technology.
...
Dividend-paying stocks exhibit lower volatility than the broad market, improving the signal-to-noise ratio of momentum signals.
\end{proposition}
...
Let $\mathcal{U} = \{s_1, s_2, \ldots, s_N\}$ denote our investment universe of $N$ securities. For each security $s_i$ at time $t$, we observe:

\begin{itemize}
    \item $P_{i,t}$: Adjusted closing price
    \item $R_{i,t}^{(L)}$: $L$-period return, defined as $R_{i,t}^{(L)} = \frac{P_{i,t}}{P_{i,t-L}} - 1$
    \item $\sigma_{i,t}^{(m)}$: $m$-period rolling volatility of daily returns
    \item $D_{i,t}$: Dividend payment on date $t$ (zero on non-payment dates)
    \item $Y_{i,t}$: Trailing dividend yield
\end{itemize}

\subsection{Momentum Calculation}

We define the momentum signal over a lookback period $L$:
...
This measure captures the total return over the trailing lookback period, consistent with optimal horizons identified in the momentum literature.
...
\begin{equation}
    \mathcal{Q}_t = \left\{ s_i \in \mathcal{A}_t : \text{MOM}_{i,t} > \gamma \cdot \sigma_{i,t}^{(m)} \right\}
    \label{eq:quality_filter}
\end{equation}

where $\gamma$ is a calibrated signal-to-noise threshold and $\sigma_{i,t}^{(m)}$ is the rolling volatility over window $m$.

\begin{remark}
The quality filter ensures that the momentum signal is statistically meaningful relative to the asset's volatility. A stock with high momentum but proportionally higher volatility may be excluded, while a stock with moderate momentum but low volatility is included.
\end{remark}
...
The portfolio selects the top $K$ securities by rank:
...
For each security, we calculate the trailing dividend yield:

\begin{equation}
    Y_{i,t} = \frac{\sum_{k=1}^{L} D_{i,t-k}}{P_{i,t}}
    \label{eq:dividend_yield}
\end{equation}

where $D_{i,t-k}$ represents dividend payments over the lookback period and $P_{i,t}$ is the current price. A floor is applied to prevent extreme weights from very low yields.

\subsection{Position Sizing: Inverse Yield Weighting}

Rather than equal weighting, the strategy employs \textbf{inverse yield-weighted} position sizing. The key insight is that within a dividend-paying universe, \textit{lower dividend yield indicates recent price appreciation}---serving as an additional momentum confirmation signal.

\begin{definition}[Inverse Yield Weight]
For each security $s_i \in \mathcal{P}_t$, the raw inverse yield score is:
\begin{equation}
    \tilde{w}_{i,t} = \frac{1}{Y_{i,t}}
    \label{eq:inverse_yield_raw}
\end{equation}
\end{definition}
...
\begin{equation}
    w_{i,t} = \lambda \cdot \frac{\tilde{w}_{i,t}}{\sum_{j \in \mathcal{P}_t} \tilde{w}_{j,t}}
    \label{eq:weights}
\end{equation}

where $\lambda$ is an optional leverage factor.

\begin{remark}[Momentum Confirmation]
Consider two stocks both selected for the portfolio:
\begin{itemize}
    \item \textbf{Stock A}: Lower yield $\rightarrow$ higher inverse score
    \item \textbf{Stock B}: Higher yield $\rightarrow$ lower inverse score
\end{itemize}
Stock A receives more weight than Stock B. The lower yield indicates Stock A's price has appreciated more relative to its dividend payments, confirming stronger momentum.
\end{remark}

\subsection{Rebalancing Protocol}

The portfolio is rebalanced at regular intervals (every $\tau$ trading days). At each rebalance date $t$:
...
\begin{enumerate}
    ...
    \item Calculate dividend yields $Y_{i,t}$ for selected securities
    \item Compute inverse yield weights per Equation \ref{eq:weights}
    \item Establish new positions with inverse yield weights
\end{enumerate}
...
The complete strategy follows this conceptual flow:

\begin{enumerate}
    \item \textbf{Initialize}: Define universe $\mathcal{U}$ and calibrated parameters
    \item \textbf{For each rebalance period}:
    \begin{enumerate}
        \item Calculate momentum and volatility for all securities
        \item Filter by absolute momentum (positive returns only)
        \item Filter by signal quality (momentum-to-volatility ratio)
        \item Rank remaining securities by momentum strength
        \item Select top performers for the portfolio
        \item Calculate dividend yields for selected securities
        \item Compute inverse yield weights (lower yield = higher weight)
        \item Execute rebalance with calculated weights
    \end{enumerate}
    \item \textbf{Monitor}: Track positions, apply risk limits
\end{enumerate}
...
The strategy generates returns from five distinct sources:
...
This measures the incremental return from selecting the top performers versus equal-weighting all qualifying stocks.
...
\subsubsection{Yield Confirmation Alpha}

The inverse yield weighting provides an additional source of alpha by amplifying exposure to stocks with the strongest price appreciation. Within a dividend-paying universe, yield serves as a normalized measure of price performance:

\begin{equation}
    \alpha_{\text{yield}} = \E\left[\sum_{i \in \mathcal{P}_t} w_{i,t}^{\text{inv}} R_{i,t+\tau}\right] - \E\left[\sum_{i \in \mathcal{P}_t} w_{i,t}^{\text{eq}} R_{i,t+\tau}\right]
    \label{eq:yield_alpha}
\end{equation}

where $w^{\text{inv}}$ denotes inverse yield weights and $w^{\text{eq}}$ denotes equal weights. This alpha arises because:

\begin{itemize}
    \item Lower yield confirms price momentum (price appreciated faster than dividends)
    \item Overweighting confirmed momentum stocks amplifies the momentum effect
    \item The signal is orthogonal to the price momentum signal itself
\end{itemize}

\subsection{Signal Quality Enhancement}

The quality filter ($\text{MOM} > \gamma \cdot \sigma$) improves the signal-to-noise ratio by eliminating false momentum signals. Consider two stocks with identical momentum:

\begin{itemize}
    \item \textbf{Stock A}: Low volatility, high ratio $\rightarrow$ \textbf{Include}
    \item \textbf{Stock B}: High volatility, low ratio $\rightarrow$ \textbf{Exclude}
\end{itemize}
...
The rebalancing period is calibrated to capture momentum persistence while managing transaction costs. The optimal rebalancing frequency balances:
...
where $V$ is trade volume, $ADV$ is average daily volume, and $\alpha$ is a constant. With high ADV requirements and positions spread across multiple holdings, market impact remains minimal.

\subsubsection{Transaction Costs}

The rebalancing frequency balances momentum capture against transaction costs:
...
\begin{itemize}
    ...
    \item \textbf{Intermediate}: Captures trends with manageable costs
\end{itemize}
...
\begin{itemize}
    \item A diversified securities universe
    \item Multiple concurrent positions
    \item Regular rebalancing events
\end{itemize}
...
With leverage, the effective beta is amplified, increasing both gains and losses relative to the market.
...
\begin{itemize}
    ...
    \item Differentiated rebalancing frequency
\end{itemize}
...
If leverage is employed, margin-related risks include:
...
The strategy's parameters were selected based on empirical analysis. Risks include:
...
\begin{itemize}
    \item Daily Value-at-Risk monitoring
    \item Maximum drawdown limits with de-leveraging triggers
    \item Sector concentration limits
    \item Single position size caps
    \item Margin utilization monitoring
\end{itemize}
...
This paper has presented a systematic equity trading strategy that combines dual momentum principles with a dividend-quality investment universe and inverse yield-weighted position sizing. The strategy addresses known weaknesses of naive momentum implementations through four key innovations:
...
\begin{enumerate}
    ...
    \item \textbf{Dividend-quality universe}: Restricting to large-cap, dividend-paying stocks reduces volatility and improves signal quality
    \item \textbf{Signal quality filter}: The momentum-to-volatility ratio threshold eliminates noisy signals
    \item \textbf{Inverse yield weighting}: Position sizing based on dividend yield provides momentum confirmation, allocating more capital to stocks with stronger price appreciation
\end{enumerate}

The strategy's theoretical foundation rests on well-documented behavioral biases---investor underreaction, the disposition effect, and limited attention---that cause prices to adjust slowly to information. The inverse yield weighting leverages a key insight: within a dividend-paying universe, lower yield indicates that price has appreciated faster than dividend payments, serving as an independent confirmation of the momentum signal. By systematically identifying and exploiting these patterns within a high-quality universe, the strategy aims to generate alpha while managing the tail risks associated with momentum investing.

Future research directions include:
\begin{itemize}
    \item Dynamic parameter adjustment based on market conditions
