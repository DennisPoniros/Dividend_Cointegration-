...
For each security, we calculate the trailing 12-month dividend yield:
...
where $D_{i,t-k}$ represents dividend payments over the lookback period and $P_{i,t}$ is the current price. A floor of 0.5\% is applied to prevent extreme weights.
...
Normalized weights with leverage factor $\lambda$ are:
...
\begin{itemize}
    \item \textbf{Stock A}: 1.5\% yield $\rightarrow$ inverse score = 66.7
    \item \textbf{Stock B}: 3.0\% yield $\rightarrow$ inverse score = 33.3
\end{itemize}
Stock A receives twice the weight of Stock B. The lower yield indicates Stock A's price has appreciated more relative to its dividend payments, confirming stronger momentum.
\end{remark}

With $\lambda = 2$ and $K = 12$ positions, the average position size is 16.67\% of capital, but individual positions vary based on relative dividend yields.
...
\begin{algorithm}[H]
\caption{Dual Momentum Strategy with Inverse Yield Weighting}
...
\For{each trading day $t$}
    ...
    \If{$d \mod \tau = 0$} \Comment{Rebalance day}
        \State \textbf{Step 1: Calculate Momentum and Volatility}
        ...
        \State \textbf{Step 5: Calculate Dividend Yields}
        \For{each $s_i \in \mathcal{P}_t$}
            \State $Y_{i,t} \gets \max\left(\frac{\sum_{k=1}^{L} D_{i,t-k}}{P_{i,t}}, 0.005\right)$ \Comment{Floor at 0.5\%}
        \EndFor

        \State \textbf{Step 6: Compute Inverse Yield Weights}
        \If{$|\mathcal{P}_t| \geq 1$}
            \For{each $s_i \in \mathcal{P}_t$}
                \State $\tilde{w}_{i,t} \gets 1 / Y_{i,t}$ \Comment{Inverse yield score}
            \EndFor
            \State $W_{\text{total}} \gets \sum_{j \in \mathcal{P}_t} \tilde{w}_{j,t}$
            \For{each $s_i \in \mathcal{P}_t$}
                \State $w_{i,t} \gets \lambda \cdot \tilde{w}_{i,t} / W_{\text{total}}$ \Comment{Normalized with leverage}
            \EndFor
        \Else
            \State Hold cash (all weights = 0)
        \EndIf

        \State \textbf{Step 7: Execute Rebalance}
...
\begin{itemize}
    ...
    \item 12 positions with inverse yield weighting
    \item 0.75 signal-to-noise threshold
    \item 0.5\% yield floor for weight calculation
...
Yield Floor & 0.5\% & Prevents extreme weights from very low yields \\
Position Weighting & Inverse Yield & Momentum confirmation via price appreciation \\
